...
const loadToasterUi = (): unknown => require('toaster-ui');

function getToaster(): ToasterUiInstance | null {
  if (typeof window === 'undefined') return null;
  if (!toasterInstance) {
    const ToasterUiModule = loadToasterUi() as { default?: unknown } | unknown;
    const ToasterUi = (ToasterUiModule as { default?: unknown })?.default ?? ToasterUiModule;
    if (typeof ToasterUi !== 'function') {
      throw new Error('toaster-ui: expected a constructor');
    }
    toasterInstance = new (ToasterUi as new () => ToasterUiInstance)();
  }
